...
export default {
  ...
  created () {
    this.getMenuList()
    this.activePath = window.sessionStorage.getItem('activePath')   //被激活导航地址高亮
  },
  methods: {
    logout () {
      // 清空token
...
.el-header {
  ...
  display: flex;
  justify-content: space-between;     // 使一行中的 两个子元素 左右对齐（两边靠）
  padding-left: 0;
  align-items: center;
  color: #fff;
  font-size: 20px;
...
.toggle-button {
  ...
  color: #fff;
  text-align: center;
  letter-spacing: 0.2em;    // 字间距   em: 相对自身字体大小来算
  // 鼠标放上去变成小手
  cursor: pointer;
}
</style>
