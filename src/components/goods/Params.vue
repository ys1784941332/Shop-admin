          ...
          <el-table :data="manyTableData" border stripe>
            ...
            <el-table-column type="expand">
              <template slot-scope="scope">
                ...
                <!-- 输入Tag文本框 -->
                <el-input class="input-new-tag" v-if="scope.row.inputVisible" v-model="scope.row.inputValue" ref="saveTagInput" size="medium"
...
export default {
  data () {
    return {
      ...
      selectedCateKeys: [],        //   级联选择框双向绑定对象
      activeTabsName: 'many',   //   激活页签名称
  ...
  methods: {
    ...
    async getParamsData () {
      ...
      const { data: res } = await this.$http.get(`categories/${this.getCateId}/attributes`,{params: { sel: this.activeTabsName }})
      if (res.meta.status !== 200) {
      ...
      if (this.activeTabsName === 'many') {        // 判断当前是哪个 选项请求的 列表数据
    ...
    async showEditDialog (attrId) {
      const { data: res } = await this.$http.get(
        `categories/${this.getCateId}/attributes/${attrId}`, { params: { attr_sel: this.activeTabsName } })
      if (res.meta.status !== 200) {
    ...
    showInput (row) {
      ...
      this.$nextTick(_ => { 
        this.$refs.saveTagInput.$refs.input.focus()         //   让输入框自动获取焦点
      })
    },
    ...
    handleInputConfirm (row) {
      ...
      row.attr_vals.push(row.inputValue.trim())   // 将输入的内容 push 到原 Tag标签的数组中
      row.inputValue = ''
      row.inputVisible = false
      // 提交数据库，保存修改
    ...
    handleClose (i, row) {
      ...
      this.saveAttrVals(row)      //提交数据库，保存修改
    }
  }
}
</script>
